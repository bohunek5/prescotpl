...
export function diffuserMap(spec,length){
  ...
  const continuous=t.type==='COB'||t.technology==='WCOB',sigma=Math.max(.0011,depth*(cover.id.includes('liger')?.95:.62)),first=points[0]||0;
  ...
  for(let x=0;x<c.width;x++){const q=sample((x/(c.width-1)-.5)*L)/normal;for(let y=0;y<c.height;y++){const cross=Math.pow(Math.sin(Math.PI*(y+.5)/c.height),.35),v=Math.round(255*Math.min(1,(.035+.965*q)*(.28+.72*cross))),i=(y*c.width+x)*4;data.data[i]=data.data[i+1]=data.data[i+2]=v;data.data[i+3]=255;}}
  ctx.putImageData(data,0,0);const texture=new T.CanvasTexture(c);texture.colorSpace=T.SRGBColorSpace;texture.anisotropy=8;return texture;
}
export function phosphorMap(white=false){
  ...
  x.putImageData(im,0,0);const texture=new T.CanvasTexture(c);texture.colorSpace=T.SRGBColorSpace;return texture;
}
